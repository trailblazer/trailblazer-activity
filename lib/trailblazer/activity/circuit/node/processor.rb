      ...
      class Node
        class Processor # DISCUSS: name?
          ...
          class Scoped < Processor
            def self.call(node, ctx, outer_lib_ctx, outer_signal, copy_from_outer_ctx: nil, return_outer_signal: false, copy_to_outer_ctx: [], context_implementation:, **circuit_options)# FIXME: we need to "cleanse" the circuit_options from any configuration, dangerous. waht if a higher node has a config that is not filtered out by the Node runner?
...
# puts "calling #{id} with signal <<<#{signal}"
              ctx, lib_ctx, signal = super(node, ctx, lib_ctx, outer_signal, **circuit_options, context_implementation: context_implementation)
            ...
            # @private
            def self.unscope(lib_ctx, outer_ctx, signal, outer_signal, return_outer_signal:, copy_to_outer_ctx:, context_implementation:)
