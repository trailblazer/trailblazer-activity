    ...
    class Circuit
      class Node < Struct.new(:id, :task, :interface, :merge_to_lib_ctx, :copy_from_outer_ctx, :copy_to_outer_ctx, :return_outer_signal, keyword_init: true) # FIXME: why does a Node have {:merge_to_lib_ctx} ?
        ...
        class Scoped < Node
          ...
          def call(ctx, outer_lib_ctx, outer_signal, context_implementation:, **circuit_options)# FIXME: we need to "cleanse" the circuit_options from any configuration, dangerous. waht if a higher node has a config that is not filtered out by the Node runner?
            ...
            lib_ctx, signal = unscope(lib_ctx, outer_lib_ctx, signal, outer_signal, context_implementation)
# puts "@@@@@ #{ctx.inspect}"
