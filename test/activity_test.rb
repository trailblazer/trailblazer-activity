...
class ActivityTest < Minitest::Spec
  describe "Activity#call" do
    it "accepts circuit interface and defaults {circuit_options}" do
      ...
      ctx, _flow_options, signal = flat_activity.call({seq: []}, {})

      assert_equal CU.inspect(ctx), %({:seq=>[:b, :c]})
      assert_equal signal.inspect, %(#<Trailblazer::Activity::End semantic=:success>)

      # b step fails.
      ctx, _flow_options, signal = flat_activity.call({seq: [], b: Trailblazer::Activity::Left}, {})
    ...
    it "accepts {:start_task}" do
      ...
      ctx, _flow_options, signal = flat_activity.call({seq: []}, {}, start_task: Implementing.method(:c))
    ...
    it "accepts {:runner}" do
      ...
      ctx, _flow_options, signal = flat_activity.call({seq: []}, {}, runner: my_runner)
    ...
    it "if a signal is not connected, it throws an {IllegalSignalError} exception with helpful error message" do
      ...
      exception = assert_raises Trailblazer::Activity::Circuit::IllegalSignalError do
        broken_activity.call({seq: []}, {}, start_task: b_task, exec_context: MyExecContext.new)
      end
