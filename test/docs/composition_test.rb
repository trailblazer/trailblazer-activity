...
class DocsCompositionTest < Minitest::Spec
  ...
  class SimpleRailwayInRailwayTest < Minitest::Spec
    ...
    it "with a plain Railway, only pass outputs" do
      # No :id
      event, (ctx,) = Memo::Update.({params: {}})
      _(event.to_h[:semantic]).must_equal :failure

      # find fails
      event, (ctx,) = Memo::Update.({params: {id: nil}})
      _(event.to_h[:semantic]).must_equal :failure

      # works
      event, (ctx,) = Memo::Update.({params: {id: 1}})
  ...
  class RailwayWithThreeEndsInRailwayTest < Minitest::Spec
    ...
    it "has outer three ends" do
      ...
      # find fails
      event, (ctx,) = Memo::Update.({params: {id: nil}})
      _(event.to_h[:semantic]).must_equal :err404
    ...
    it "connects :id_missing to :failure" do
      # No :id
      event, (ctx,) = Rewire::Memo::Update.({params: {}})
      _(event.to_h[:semantic]).must_equal :failure

      # find fails
      event, (ctx,) = Rewire::Memo::Update.({params: {id: nil}})
      _(event.to_h[:semantic]).must_equal :err404

      # works! :success
      event, (ctx,) = Rewire::Memo::Update.({params: {id: 1}})
      _(event.to_h[:semantic]).must_equal :success
      _(ctx[:model].to_h.inspect).must_equal %{{:id=>1, :body=>\"Yo!\"}}
    end
    ...
    it "connects :id_missing to :failure" do
      # No :id
      event, (ctx,) = Subprocess::Memo::Update.({params: {}})
      _(event.to_h[:semantic]).must_equal :failure

      # find fails
      event, (ctx,) = Subprocess::Memo::Update.({params: {id: nil}})
      _(event.to_h[:semantic]).must_equal :err404

      # works! :success
      event, (ctx,) = Subprocess::Memo::Update.({params: {id: 1}})
      _(event.to_h[:semantic]).must_equal :success
      _(ctx[:model].to_h.inspect).must_equal %{{:id=>1, :body=>\"Yo!\"}}
    end
  end
